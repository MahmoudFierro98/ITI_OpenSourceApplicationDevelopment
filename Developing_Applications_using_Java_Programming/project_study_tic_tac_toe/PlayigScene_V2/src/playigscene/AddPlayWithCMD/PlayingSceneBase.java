...
public class PlayingSceneBase extends AnchorPane implements Runnable {
    ...
    protected final CheckBox recordAgainCheckBox;
    protected final Label gameRecordedSuccessfullyLabel;
    ...
    public PlayingSceneBase(String playModeCopy, String player1NameCopy, String player2NameCopy)
    {
        ...
        recordAgainCheckBox = new CheckBox();
        gameRecordedSuccessfullyLabel = new Label();
        ...
        gamePointsLabel.setLayoutY(119.0);
        ...
        recordAgainCheckBox.setLayoutX(201.0);
        recordAgainCheckBox.setLayoutY(279.0);
        recordAgainCheckBox.setMnemonicParsing(false);
        recordAgainCheckBox.setText("Record?");
        recordAgainCheckBox.setTextFill(javafx.scene.paint.Color.WHITE);
        recordAgainCheckBox.setFont(new Font("System Bold", 28.0));

        gameRecordedSuccessfullyLabel.setLayoutX(152.0);
        gameRecordedSuccessfullyLabel.setLayoutY(27.0);
        gameRecordedSuccessfullyLabel.setText("Game Recorded Successfully");
        gameRecordedSuccessfullyLabel.setTextFill(javafx.scene.paint.Color.valueOf("#54b228"));
        gameRecordedSuccessfullyLabel.setFont(new Font("System Bold", 19.0));
        gameRecordedSuccessfullyLabel.setVisible(false);
        ...
        gameStatePane.getChildren().add(recordAgainCheckBox);
        gameStatePane.getChildren().add(gameRecordedSuccessfullyLabel);
    ...
    /* setRecord method to set steps of the game */
    public void setRecord(String gameRecordDisplayCopy)
    {
        gameRecordDisplay = gameRecordDisplayCopy;
        for(int i=0;i<gameRecordDisplay.length(); i++)
        {
            initializeRecordedGame(Character.getNumericValue(gameRecordDisplay.charAt(i)));
        }
        playerSide = 'z';
        setPlayersNameAndPoints();
        startRecordedGame();
    }
    /* initializeRecordedGame method to initialize playButtons with steps */
    private void initializeRecordedGame(int btnNumber)
    {
        if(turnX)
        {
            playButton[btnNumber].setText("X");
            playButton[btnNumber].setTextFill(javafx.scene.paint.Color.valueOf("#e4a593"));
            state[btnNumber] = 'x';
            turnX = false;
        }
        else
        {
            playButton[btnNumber].setText("O");
            playButton[btnNumber].setTextFill(javafx.scene.paint.Color.valueOf("#5b3379"));
            state[btnNumber] = 'o';
            turnX = true;
        }
        playButton[btnNumber].setVisible(false);
    } 
    /* startRecordedGame method to start thread of record */
    ...
    /* */
    private void displayRecordedGame(int btnNumber) 
    {
        playButton[Character.getNumericValue(gameRecordDisplay.charAt(btnNumber))].setVisible(true);
        System.out.println("playButton["+Character.getNumericValue(gameRecordDisplay.charAt(btnNumber))+"]: "+state[Character.getNumericValue(gameRecordDisplay.charAt(btnNumber))]);
        if(btnNumber == gameRecordDisplay.length()-1)
        {
            if(gameRecordDisplay.length() == 9)
            {
                counter = 9;
            }
            checkState();
    ...
    public void run()
    {
        for(int i=0;i<gameRecordDisplay.length(); i++)
        {
            try{
                ...
                Logger.getLogger(PlayingSceneBase.class.getName()).log(Level.SEVERE, null, ex);
            }
            displayRecordedGame(i);
        }
    }
    ...
    private void checkGameMode()
    {
        ...
        {
            playerSide = 'x';setPlayersNameAndPoints();
        }
        ...
        else if(playMode.equals("RecordedGame"))
        {   
            System.out.println("Recorded Game");
        }
    }
    ...
    private void setPlayersNameAndPoints()
    {
        ...
        else if((playMode.equals("RecordedGame")) || (playMode.equals("vsFriendSendRequest")))
        {
                player1NameLabel.setText(player1Name);
                player2NameLabel.setText(player2Name);
                player1PointsLabel.setText("");
    ...
    private void playvsFriendSendRequest()
    {
        ...
        if(!finishFlag)
        {
            ...
            {
                int j = i;
                playButton[i].setOnMouseMoved(event -> {
                    if(!turnX)
                    {
                        playOFriend();
                    }
    ...
    private void playvsFriendReceiveRequest()
    {
    ...
    private void playXFriend()
    {
        ...
        {   
            do{ 
                btnNumber = scan.nextInt();
            }while(state[btnNumber] != 'z');
            playX(playButton[btnNumber],btnNumber);
        }
    } 

    private void playOFriend()
    {
        Scanner scan = new Scanner(System.in);
        int btnNumber;
        if((counter < 9) && !finishFlag)
        {   
            ...
            playO(playButton[btnNumber],btnNumber);playButton[btnNumber].fire();
        }
    } 

    private void checkRecord()
    {
        if(recordCheckBox.isSelected() || recordAgainCheckBox.isSelected())
        {
            recordCheckBox.setSelected(false);
            recordAgainCheckBox.setSelected(true);
            gameRecordedSuccessfullyLabel.setVisible(true);
            recordGame();
        }
        else
        {
